...
\Statex \Comment used by FP writes
%\Atomic 
	\If{key has no tentative version}
	\If{last committed version of key $\le$ old}
		\State ver $\leftarrow$ F\&I(key's maxVersion)   \label{l:fi}
...
Thus, $T_1$'s write  detects the conflict on $x$. 
 ...
 Note that this modification of transactional reads and post-commits incurs an extra cost on
