...
% 			\State $ts_c \leftarrow$ {\sc checkStatus}(rec, $ts_r$)     			       
			...
			\Else \Comment commit cell is empty, check CT
      				...
      				\If{$ts_c =$ nil}  \Comment no CT entry, set status to  $\bot$
      					\State \inred{$ts_c\leftarrow$CT.check\&mutate(rec.ts, commit, nil, $\bot$)}  
...
\Procedure{commit}{$ts_r$, write-set}
      	...
      	\If{$ts_c \not=$ abort}
	      	\inred{\If {CT.check\&mutate($ts_r$, commit, nil, $ts_c$) $=\bot$}} % \Comment Set field only if field is missing and return $\bot$ if transaction $ts_r$ was aborted}
%   		\If{$\neg$ok} 
...
order to allow the transaction to complete without aborting it.
...
unexpected network or processing delays we take precautions to avoid correctness
violations in cases when both primaries are active at the same time.
...
As with locks, at most one TM may hold the lease at a given time. 
