...
\Yoni{This is a bad example. I think we can find an example with one FP and one Tx}
\begin{itemize}
...
One option is to abort an FP transaction whenever a write intent exists for one of its written or read keys.\Yoni{read keys? In current implementation we dont fail we just look for a commited version} 
