...
infrastructure -- even web search. For example, Phoenix is designed to scale to 10K query processing 
nodes in one instance. Such expectations push the scalability limits of the transaction processing 
infrastructure to hundreds of thousands or even millions transactions per second (tps). 
 }}
...
{\inred{Omid is database-neutral, i.e., it could use any data store with 
traditional NoSQL API (see Section~\ref{sec:api}). For example, it does 
not require any changes to HBase code.  }}
...
{\inred{We also extend the traditional SI model, which provides a single-read-point 
semantics, with multiple read points, which helps to avoid recursive 
read-your-own-writes scenarios in complex queries.}} 

\item {\inred {{\em Design space\/}. We explore the design choices of transaction processing engines 
for NoSQL data stores, highlighting the tradeoffs between them. In particular, we compare Omid's 
approach based on a centralized conflict tie breaker to the two-phase commit (2PC) approach
adopted by Percolator~\cite{Percolator2010} and CockroachDB~\cite{cockroach}.
We implement a variant of Omid, named \syspc, which follows Percolator's protocol, 
and compare its properties to our solution.}}
