...
% 			\State $ts_c \leftarrow$ {\sc checkStatus}(rec, $ts_r$)     			       
			...
			\Else \Comment commit cell is empty, check CT
      				...
      				\If{$ts_c =$ nil}  \Comment no CT entry, set status to  $\bot$
      					\State $ts_c \leftarrow$ CT.putIfAbsent(rec.ts, $\bot$)  %\Comment returns new value
      					\If{$ts_c$=nil} \Comment putIfAbsent succeeded 
...
an atomic putIfAbsent to create the commit entry \inred{in the CT}.
...
\inred{This ensures that no client will obtain a commit timestamp in an old epoch after the new TM has started using a new one; in particular, it preserves the invariant that once a new TM has started all updates by clients that obtain a commit timestamp in an old epoch are in the data store.}
