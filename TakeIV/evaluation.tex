...
We also do no compare \sys\ to Omid1 and Tephra, since both significantly
...
\begin{tabular}{cc}
	...
	\caption[]{\small Throughput vs latency, transaction size=10}
    \label{fig:tl-10}
  \end{subfigure} & 
  ...
  \begin{subfigure}[t]{0.35\textwidth}
	...
	\caption[]{\small Latency breakdown, transaction size = 5, 10}
    \label{fig:stack-tx10}
  \end{subfigure} 
\end{tabular}  	
}
\vspace{-0.3cm}
  \caption{\small Latency vs.\ throughput  and latency breakdown  for long transactions in random mix workload. }
  \vspace{-0.3cm}
...
Under high load (Figure~\ref{fig:slowdown-high}), the fast path speedup for writes is $2.7$x.
%In systems geared towards real-time processing, this is a reasonable tradeoff, since long transactions 
