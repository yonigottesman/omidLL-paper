...
Returns an iterator that supports retrieval of multiple records using getNext() calls.
}}
...
\item [{check\&mutate(key, field, old, new)}] -- checks the record associated with key. 
\inred{If field holds old, replaces it with new; either way returns field's previous value.}
\inred{If old is nil, creates field and initializes it with new only if key is missing in data store.}
...
\noindent
{\inred{Most NoSQL data store implementations guarantee atomic execution of each of the above API's, except traversals of iterators returned by scan.}
