...
CockroachDB takes a different approach: it performs distributed validation at write time, by replacing writes with atomic validate-and-write operations that can abort either the current transaction or a conflicting one. 
...
an atomic check\&mutate operation to set its status to aborted. Similar scenarios occur in Percolator and CockroachDB, since 
...
Such tight coupling between validation and timestamp allocation comes at the cost of 
delaying begin requests in Omid, and sending information about many previous transactions to the client in Omid1 and Tephra.
