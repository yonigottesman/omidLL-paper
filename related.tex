...
rather than adding an FP API. It has a less efficient implementation, and moreover, 
Mediator's  native operations never concede to transactions, which can starve in case 
of contention with native operations. 

Our implementation of the fast path uses a local version clock. Similar mechanisms
were used in Mediator as well as in CockroachDB, which uses 
