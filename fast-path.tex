...
\Procedure{bwc}{key, value} 
	...
	\State TODO take into account maxVersion
	\If{ver = nil}  \Comment found tentative value -- abort 
		\State return abort 
	\EndIf
	...
	ver $\leftarrow$ ver $+1$ 
\Atomic 
	\If{last committed version of key = ver$-1$}
	\State ds.put(\tuple{key, ver} with commit = ver)  
	\State $maxVersion \leftarrow \max(maxVersion, ver)+1$
...
\Statex
\Procedure{br}{key} 
\State TODO % $\langle$ \code{ts$_r$}, value  $\rangle \leftarrow$ getWithTs(key)
\State  return \tuple{$ts_r$, value}
\EndProcedure
...
\inred{Todo: add \code{br} and \code{wc}}
