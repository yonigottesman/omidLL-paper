...
CockroachDB~\cite{cockroach} takes a different approach: it performs distributed validation at write time, by replacing writes with atomic validate-and-write operations that can abort either the current transaction or a conflicting one. 
...
by an uncommitted transaction, that transaction will not commit with an earlier timestamp than the read.
This is achieved by a single TM that allocates timestamps, performs validation, and writes the commit entry,
and either delays begin requests until all transactions that concurrently attempt to commit complete (as in Omid)
