...
The generic fast  path algorithm is given in Algorithm~\ref{alg:fp}. 
%
...
reserved for sequence numbers assigned by FP writes, and the the most significant bits representing 
...
because it uses a form of two-phase locking: when check\&mutate begins, its locks the key, then the access to the LVC effectively
locks it for the duration of its update; this is followed by an update of the key's record and the key's lock being released.
