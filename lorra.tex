...
% by distributing the commit entries.
...
We will see that, while many TPSs follow a similar schema,  they make different design choices when implementing the schema. 
...
\Else
	\State abort	
\EndIf
\State post-commit: uppdate meta-data
\EndProcedure
...
In Omid, the version is the transaction's $ts_r$, wihch exceeds all versions written by transactions that committed before the
...
In some cases~\cite{Percolator2010,cockroach}, when the writing transaction status is undertermined, the read may forcefully abort
...
\item \emph{Commit} or abort in one  irrevocable atomic step. This is achieved by persistently writing to the \emph{commit entry}, 
 ...
 \item \emph{Post-commit}: 
  ...
  and can be done asyncrhonously.
  %in
...
Since clients may stall or fail while holding a lock, Percolator times-out on unresponsive clients and aborts transactions on their behlaf. 
...
\inred{by up to an order of magintude} on small transactions.
