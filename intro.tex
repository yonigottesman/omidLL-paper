...
NOSQL key-value stores such as \Yoni{cite here and only here?}BigTable, HBase, and RocksDB have increased in popularity over the past few years, and are now very widely deployed. Yet in many installations, the single-key access offered by these data stores is insufficient; developers  increasingly require 
...
Our main goal is to expedite short single-object \Yoni{what is a single object?} transactions,
which are popular in production web workloads~\cite{omid-blog}.

\Yoni{Add a motivation paragraph. Some kind of workload that can benifit from local trnasactions. And a workload that would benifit from br-*r-wc}
