...
similarly. Such programs give rise to the pattern given in Algorithm~\ref{alg:sql}. 

\begin{algorithm}[h]
\begin{algorithmic}
\For{iterations $i=1, \dots$}
\State insert into T as select func(T.rec) from T  where ... 
% \Comment loop over entries of T 
\EndFor
\end{algorithmic}
\caption{SQL program that requires SIX isolation when reading from the same table it inserts to.} 
\label{alg:sql}
\end{algorithm} 

Note that the SQL statement loops over entries of $T$; in other words, it is a nested loop. 
...
Given these two calls, Phoenix translates SQL statements as above to the loop in algorithm~\ref{alg:six}-- 
...
\EndFor
\end{algorithmic}
\caption{Calls generated by Phoenix for the SQL program in Algorithm~\ref{alg:sql}.} 
\label{alg:six}
\end{algorithm} 
