...
\inred{This ensures that no client will obtain a commit timestamp in an old epoch after the new TM has started using a new one; in particular, it preserves the invariant that once a client obtains a commit timestamp $ts_c$, all updates by transactions with commit timestamps smaller than $ts_c$ are in the data store.}
