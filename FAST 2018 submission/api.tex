...
\begin{description}
\item [\code{get(key, versions,}]{\bf[field])} --  Returns the value of field in requested version(s) of the requested key; 
%if no version is provided, returns the latest; 
if no field is provided returns the entire record. 
%\item The API further allows traversing (reading) earlier versions of the same key in descending order.
