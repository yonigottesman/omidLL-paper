...
which facilitates production deployment.
...
underlying data store to allow (1) transactional reads to abort lock-holding transactions; and 
(2) transaction validation (conflict detection) using check\&mutate for each written key. The latter is slower than data store writes,
