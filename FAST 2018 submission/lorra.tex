...
in this section we describe \sys\ without the fast-path; this protocol satisfies standard (unrelaxed) SI semantics.
...
We are not familiar with another TPS that makes the same design choices as \sys. 
...
Percolator, the first TPS in this vein, used a two-phase commit protocol that locked all written objects during validation.
While Percolator did this at commit time, CockroachDB~\cite{cockroach} performs distributed validation at write time, 
by replacing writes with atomic validate-and-write operations that can abort either the current transaction or a conflicting one. 
This approach slows down either commits (in case of commit-time validation as in Percolator) or 
transactional writes (in case of write-time validation). 
In \sys, we instead adopt Omid's centralized conflict detection mechanism, which is extremely scalable~\cite{Omid2017}. 
...
We therefore distribute the commit table: we  
